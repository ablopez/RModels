...
---
# Introduction
This document describe the analysis to complete the final project for the course "Regression Models".
...
Note: Some code is shown throuhgout the exec summary to assist understanding the info required to generate the results under each section. Plots/Full computaions are found in Appendix.
...
First, I  analyse the date set 'mtcars' to explore the relationship between all the variables andmiles per gallon (MPG) using criteria: 
...
b) However, the calculations show that the are higher correlation with the variables wt, disp, cyl and hp.Figure 2 - pair shows such correaltion and supports argument 1. 
Further analysis to these variables must be then performed to answer the questions.
...
- Please Refer to Routine 1 Determing relevant variables
Please refer to routine 1 in Appendix.
...
As shows in table above p-value = .00137
...
# Regression Analysis
This analyis includes 4 models ass described below:
...
Routine 1. Calculates and extract the relevant variables
```{r}
data(mtcars)
mpgcor <- abs(data.frame(cor(mtcars))[1, ]);mpgcor1 <- order(mpgcor, decreasing = TRUE)
mpgcor2<- mpgcor1[1:5]; names(mtcars)[mpgcor2][1:5]
